...
updates_dir=/data/xperience_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have XPerience-VERSION-DATE-TYPE-* as format"
...
# Assume XPerience-VERSION-DATE-TYPE-*.zip
...
adb shell "killall mx.xperience.updater 2>/dev/null"
adb shell "sqlite3 /data/data/mx.xperience.updater/databases/updates.db" \
